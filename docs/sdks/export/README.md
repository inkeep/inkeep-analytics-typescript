...
async function run() {
  const result = await inkeepAnalytics.export.export({
...
import { exportExport } from "@inkeep/inkeep-analytics/funcs/exportExport.js";
...
async function run() {
  const res = await exportExport(inkeepAnalytics, {
    table: "semantic_threads_view",
    requestBody: {},
  });
